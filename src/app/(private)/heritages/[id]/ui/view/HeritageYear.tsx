    ...
    <div>
      ...
      >
        <div className='bg-white rounded shadow-sm p-4'>
          ...
          {data &&
            data.balances.map((balance: {code: string, total_amount: number}) => (
              <div className='flex items-center justify-end' key={balance.code}>
                <Typography
                  variant='h5'
                  className={
                    balance.total_amount > 0 ? 'text-green-500' : 'text-red-500'
                  }
                >
        ...
        <div className='bg-white rounded shadow-sm p-4'>
          ...
          {data &&
            data.investments.map((investment: {code: string, total_end_amount: number}) => (
              <div className='flex items-center justify-end' key={investment.code}>
                <Typography
                  variant='h5'
                  className={
                    investment.total_end_amount > 0 ? 'text-green-500' : 'text-red-500'
                  }
                >
                  {formatCurrency.format(investment.total_end_amount)} {investment.code}
                </Typography>
              </div>
            ))}
        </div>
