...
export default function useVerify() {
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      const { getVerifyEmail } = new AuthUseCase(new AuthApiAdapter({baseUrl: process.env.NEXT_PUBLIC_API_URL ?? '', customConfig: customConfigHeader(),}));
      if(id && hash && expires && signature) {
        ...
        if(user) {
          ...
          localStorage.setItem('emma-user', JSON.stringify(userjson))
