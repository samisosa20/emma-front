...
class AuthApiAdapter implements AuthAdapter {
  ...
  async postLogin({ email, password }: Login): Promise<Auth> {
    const result: any = await this.httpService.post<Login>(`auth/login`, {
      email,
      password,
    });
    if (result.error) {
    ...
    return {
      ...
      verify_email: !!result.confirmedEmailAt,
      token: result.token,
      currency: result.data.badgeId,
      transfer_id: result.data.transferId,
      accounts_type: result.accountsType.map((t: any) => {
        return { label: t.name, value: t.id };
      }),
      currencies: result.badges.map((c: any) => {
        return { label: c.code, value: c.id };
      }),
      groups_category: result.groupsCategory.map((c: any) => {
  ...
  async postRegister(data: Register): Promise<Auth> {
    const result: any = await this.httpService.post<Register>(
      `auth/register`,
      data
    );
    if (result.error) {
    ...
    return {
      ...
      currency: result.data.badgeId,
      transfer_id: result.data.transferId,
      accounts_type: result.accounts_type.map((t: any) => {
        return { label: t.name, value: t.id };
      }),
      currencies: result.currencies.map((c: any) => {
        return { label: c.code, value: c.id };
      }),
      groups_category: result.groups_category.map((c: any) => {
        return { label: c.name, value: c.id };
      }),
      periods: Object.entries(result.periods).map(([key, label]) => ({
        value: key,
  ...
  async getVerifyEmail(
    ...
    const result: any = await this.httpService.get(
      `/auth/verify/${id}/${hash}?expires=${expires}&signature=${signature}`
    );
